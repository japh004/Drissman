                ...
                <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-white/5 rounded-xl border border-white/5">
                            <div className="h-7 w-7 rounded-lg bg-gradient-to-br from-signal/20 to-green-500/20 flex items-center justify-center text-signal font-bold text-[10px]">
                                {user.firstName?.[0]}{user.lastName?.[0]}
                            </div>
                            <div>
                                <span className="text-xs font-bold text-snow block">{user.firstName}</span>
                                <span className="text-[9px] text-mist/40">Moniteur</span>
                            </div>
                        </div>
                ...
                {mobileMenuOpen && (
                    <div className="sm:hidden bg-asphalt/95 backdrop-blur-lg border-t border-white/[0.06] px-4 py-3 space-y-1">
                        <div className="flex items-center gap-2 px-3 py-2 mb-2">
                            <div className="h-8 w-8 rounded-xl bg-gradient-to-br from-signal/20 to-green-500/20 flex items-center justify-center text-signal font-bold text-xs">
                                {user.firstName?.[0]}{user.lastName?.[0]}
                            </div>
                            <div>
                                <p className="text-sm font-bold text-snow">{user.firstName} {user.lastName}</p>
                                <p className="text-[10px] text-mist/40">Moniteur · {user.email}</p>
                            </div>
                        </div>
                        {navItems.map((item) => {
                            const isActive = pathname === item.href;
                ...
                {navItems.map((item) => {
                    ...
                    return (
                        <Link key={item.href} href={item.href}
                            ...
                            <span className="text-[9px] font-bold">{item.name.replace("Mon ", "").replace("Mes ", "")}</span>
                        </Link>
                    );
                })}
            </nav>
