...
import { Loader2, Compass, Search, Building2, LogOut, Car, Menu, X } from "lucide-react";
...
export default function VisitorLayout({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        if (!isLoading && (!isAuthenticated || user?.role !== "VISITOR")) {
        ...
        <div className="min-h-screen bg-asphalt pb-20 sm:pb-0">
            <header className="sticky top-0 z-30 bg-asphalt/80 backdrop-blur-lg border-b border-white/[0.06]">
                <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
                    <Link href="/visitor" className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-white/5 rounded-xl border border-white/5">
                            <div className="h-7 w-7 rounded-lg bg-gradient-to-br from-emerald-500/20 to-blue-500/20 flex items-center justify-center text-emerald-400 font-bold text-[10px]">
                ...
                {mobileMenuOpen && (
                    <div className="sm:hidden bg-asphalt/95 backdrop-blur-lg border-t border-white/[0.06] px-4 py-3 space-y-1">
                        <div className="flex items-center gap-2 px-3 py-2 mb-2">
                            <div className="h-8 w-8 rounded-xl bg-gradient-to-br from-emerald-500/20 to-blue-500/20 flex items-center justify-center text-emerald-400 font-bold text-xs">
                                {user.firstName?.[0]}{user.lastName?.[0]}
                            </div>
                            <div>
                                <p className="text-sm font-bold text-snow">{user.firstName} {user.lastName}</p>
                                <p className="text-[10px] text-mist/40">Visiteur · {user.email}</p>
                            </div>
                        </div>
                        {navItems.map((item) => {
                            const isActive = pathname === item.href;
