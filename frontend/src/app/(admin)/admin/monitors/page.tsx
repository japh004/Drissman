...
import { Plus, Search, Edit2, Trash2, Phone, Mail, ShieldCheck, User } from "lucide-react";

const mockMonitors = [
    { id: "1", firstName: "Jean-Paul", lastName: "Mbarga", email: "jp.mbarga@drissman.com", phone: "+237 690 123 456", specialties: "Permis B, Conduite Accompagnée", status: "ACTIVE", sessionsThisWeek: 12, avatarUrl: "" },
    { id: "2", firstName: "Marie", lastName: "Douala", email: "m.douala@drissman.com", phone: "+237 677 456 789", specialties: "Code, Permis B", status: "ACTIVE", sessionsThisWeek: 8, avatarUrl: "" },
    { id: "3", firstName: "Pierre", lastName: "Yaoundé", email: "p.yaounde@drissman.com", phone: "+237 655 789 012", specialties: "Permis A, Permis B", status: "ON_LEAVE", sessionsThisWeek: 0, avatarUrl: "" },
];
...
export default function MonitorsPage() {
    ...
    const filtered = mockMonitors.filter(m =>
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-mist mt-0.5">{mockMonitors.length} moniteurs</p>
                </div>
                <button
                    onClick={() => { setShowModal(true); setFormFirstName(""); setFormLastName(""); setFormEmail(""); setFormPhone(""); setFormSpecialties(""); }}
                        ...
                        <div key={mon.id} className="bg-white/[0.03] rounded-2xl border border-white/[0.06] p-5 hover:border-white/10 transition-all group">
                            <div className="flex items-start gap-4 mb-4">
                                ...
                                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    ...
                                    <button className="p-1.5 rounded-lg hover:bg-red-500/10 text-mist hover:text-red-400 transition-all"><Trash2 className="h-3.5 w-3.5" /></button>
                                </div>
                            </div>
                            <div className="space-y-2 text-xs text-mist/60">
                                ...
                                <div className="flex items-center gap-2"><Phone className="h-3.5 w-3.5" />{mon.phone}</div>
                                <div className="flex items-center gap-2"><ShieldCheck className="h-3.5 w-3.5" />{mon.specialties}</div>
                            </div>
                    ...
                    <div className="bg-asphalt border border-white/10 rounded-2xl w-full max-w-lg p-6">
                        ...
                        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); setShowModal(false); }}>
                            <div className="grid grid-cols-2 gap-3">
                                <div className="space-y-1.5"><label className="text-xs font-bold text-mist uppercase tracking-wider">Prénom</label>
                                    <input type="text" value={formFirstName} onChange={(e) => setFormFirstName(e.target.value)} placeholder="Jean-Paul"
                                        className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-snow placeholder:text-mist/40 focus:outline-none focus:border-signal/50 focus:ring-2 focus:ring-signal/20 transition-all text-sm" required /></div>
                            ...
                            <div className="flex items-center gap-3 pt-2">
                                ...
                                <button type="submit"
                                    className="flex-1 py-3 rounded-xl bg-gradient-to-r from-signal to-amber-400 text-asphalt text-sm font-black hover:opacity-90 transition-all shadow-lg shadow-signal/20">Créer le moniteur</button>
                            </div>
                        </form>
                    </div>
                </div>
